...
function ImportTab() {
  ...
  const handleUpload = async () => {
    ...
    setUploading(true)
    try {
      // TODO: Implement actual .skill file upload and parsing
      // This would involve:
      // 1. Upload file to backend
      // 2. Backend unzips and parses SKILL.md
      // 3. Extracts metadata and files
      // 4. Creates Skill via API
      ...
      // await api.uploadSkillPackage(formData)
...
function GenerateTab() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      // Construct the skill-creator prompt
...
- Follow the progressive disclosure principle`
      ...
      // Submit task to selected session
        ...
        try {
          ...
          if (taskStatus.status === 'completed') {
            ...
            try {
              ...
              const jsonMatch = taskStatus.result?.match(/```json\n([\s\S]*?)\n```/) ||
                               taskStatus.result?.match(/({[\s\S]*})/)
            ...
            clearInterval(pollInterval)
            toast.error('Skill generation failed: ' + (taskStatus.error || 'Unknown error'))
            setStep('input')
          }
        } catch (err) {
          clearInterval(pollInterval)
  ...
  return (
    <div className="space-y-6">
          ...
          >
            ...
            {runningSessions.map(session => (
              <option key={session.id} value={session.id}>
                {session.profile_name} ({session.id.slice(0, 8)})
              </option>
            ))}
          </select>
          <p className="text-xs text-muted mt-1">
            The selected agent will generate your skill
          </p>
        </div>
      )}
      ...
      <div className="card p-6">
        <label className="block text-sm font-medium text-secondary mb-2">
          Describe Your Skill Requirements *
        </label>
