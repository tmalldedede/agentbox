      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="font-semibold text-primary truncate">{server.name}</span>
            {server.is_built_in && (
          ...
          <div className="flex items-center gap-2 mt-3 flex-wrap">
            ...
            {server.tags?.slice(0, 3).map(tag => (
              <span key={tag} className="text-xs px-2 py-0.5 rounded bg-secondary text-muted">
                {tag}
              </span>
            ))}
          </div>
            ...
            {categories.map(category => (
              <div key={category}>
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <span className="text-sm text-muted">({groupedServers[category].length})</span>
                </div>
