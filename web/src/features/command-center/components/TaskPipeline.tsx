    ...
    <div className="flex h-full flex-col rounded-xl border border-border bg-card/50 backdrop-blur-sm">
      <div className="flex items-center justify-between border-b border-border px-5 py-4">
        <div className="flex items-center gap-2">
          <GitPullRequest className="text-yellow-500" size={16} />
          <h3 className="font-semibold text-foreground">Task Pipeline</h3>
        </div>
      ...
      <div className="flex flex-1 flex-col overflow-y-auto custom-scrollbar p-0">
        {tasks.map((task, idx) => {
          ...
          return (
            <div key={task.id} className="relative pl-6 pr-4 py-3 group hover:bg-accent/30 transition-colors">
              {/* Vertical Timeline Line */}
              {idx !== tasks.length - 1 && (
                <div className="absolute left-[29px] top-8 bottom-[-12px] w-px bg-border group-hover:bg-accent transition-colors" />
              )}
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex min-w-0 flex-1 flex-col gap-1.5 pt-0.5">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <span className="shrink-0 text-[10px] font-mono text-muted-foreground bg-accent/50 px-1.5 py-0.5 rounded">
                      {task.elapsed}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-3 text-xs text-muted-foreground">
                    <div className="flex items-center gap-1.5">
                      <div className={`h-1.5 w-1.5 rounded-full ${task.status === 'running' ? 'bg-blue-500 animate-pulse' : 'bg-muted-foreground/30'}`} />
                      <span className={`capitalize ${config.color} text-[11px] font-medium`}>{task.status}</span>
                    </div>
                    <span className="text-border">|</span>
                    <span className="text-[11px] truncate max-w-[120px]">
                      Agent: <span className="text-muted-foreground">{task.agent}</span>
                    </span>
                  </div>
