    ...
    <div className="flex h-full flex-col rounded-xl border border-zinc-800 bg-zinc-900/50 backdrop-blur-sm">
      <div className="flex items-center justify-between border-b border-zinc-800 px-5 py-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-zinc-100">Active Executions</h3>
        </div>
        <span className="flex items-center gap-1.5 rounded-full bg-purple-500/10 border border-purple-500/20 px-2.5 py-1 text-[10px] font-bold text-purple-400">
      ...
      <div className="flex flex-1 flex-col gap-3 overflow-y-auto p-4 custom-scrollbar">
        {executions.map((exec) => (
          <div 
            key={exec.id} 
            className="group relative flex flex-col gap-3 rounded-xl border border-zinc-800 bg-zinc-900/40 p-4 transition-all hover:border-zinc-700 hover:bg-zinc-800/60"
          >
            {/* Header */}
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-start gap-2.5 min-w-0">
                <div className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded bg-zinc-800 text-zinc-400">
                  <Terminal size={10} />
                </div>
                <span className="text-xs font-medium text-zinc-200 leading-relaxed line-clamp-2" title={exec.prompt}>
                  {exec.prompt}
                </span>
              </div>
              <span className="shrink-0 text-[10px] font-medium text-zinc-500 bg-zinc-800/50 px-2 py-1 rounded-md border border-zinc-800 font-mono">
                ETA {exec.eta}
              </span>
            </div>
            ...
            <div className="flex flex-col gap-2">
               <div className="flex items-center justify-between text-[11px]">
                  ...
                  <span className="font-mono font-bold text-purple-400">{exec.progress}%</span>
               </div>
        ...
        {executions.length === 0 && (
          <div className="flex flex-1 items-center justify-center text-xs text-zinc-600 font-medium">
