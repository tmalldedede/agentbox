...
import { Route } from '@/routes/_authenticated/profiles/$id'
import {
  ArrowLeft,
  Save,
  Trash2,
...
export default function ProfileDetail() {
  ...
  // Get ID from route params or use 'new' for new profile
  const params = Route.useParams()
  const profileId = params?.id || 'new'
  ...
  const handleSave = async () => {
    ...
    if (isNewProfile) {
      createProfile.mutate(profileData as any, {
        onSuccess: created => {
          setTimeout(() => navigate({ to: `/profiles/${created.id}` }), 500)
        },
      })
    } else {
  ...
  if (profileError && !isNewProfile) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center">
        ...
        <p className="text-lg text-foreground">Profile not found</p>
        <button onClick={() => navigate({ to: '/profiles' })} className="btn btn-primary mt-4">
          Back to Profiles
        </button>
      </div>
    )
    ...
    <div className="min-h-screen">
      ...
      <header className="app-header">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate({ to: '/profiles' })} className="btn btn-ghost btn-icon">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {isBuiltIn && (
                <span className="ml-2 text-xs px-2 py-0.5 rounded bg-secondary text-muted-foreground">
